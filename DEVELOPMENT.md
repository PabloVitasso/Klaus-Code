...
For fully automated releases, use the following process.

#### Prerequisites

1. **Create a GitHub PAT** with `repo` scope at https://github.com/settings/tokens
2. **Configure gh CLI** with the token:

```bash
# Unset any conflicting env var
unset GH_TOKEN

# Authenticate with gh using the PAT
gh auth login --hostname github.com --with-token ghp_your_pat_here
```

3. **Build the VSIX first**:

```bash
pnpm clean && pnpm build && pnpm vsix
```
...
# Run with version (or auto-detect from latest tag)
./scripts/release.sh 3.42.0-klaus.2
```
...
# Create PR
gh pr create --repo PabloVitasso/Klaus-Code --base main \
  --head PabloVitasso:release/v3.42.0-klaus.2 \
  --title "Release v3.42.0-klaus.2" --body "Release v3.42.0-klaus.2"

# Merge PR (get PR number from output or gh pr list)
gh pr merge 2 --merge --admin --repo PabloVitasso/Klaus-Code

# Create tag
git checkout main && git pull origin main
git tag -a v3.42.0-klaus.2 -m "Release v3.42.0-klaus.2"
git push origin v3.42.0-klaus.2

# Create GitHub release
gh release create v3.42.0-klaus.2 --title "v3.42.0-klaus.2" \
  --repo PabloVitasso/Klaus-Code --notes "## What's New..."
gh release upload v3.42.0-klaus.2 bin/klaus-code-3.42.0-klaus.2.vsix \
  --repo PabloVitasso/Klaus-Code
```
