...
Klaus Code uses **cherry-picking** to merge upstream commits, which creates new commit hashes:
...
### Strategy-Ours Merge (2026-01-25)

To properly record the merge in git history and fix GitHub's "behind" counter, a strategy-ours merge was performed:

```bash
git merge -s ours 953c7773c
```

**Commit:** `e61d32b01` - Merge Roo Code upstream commits up to 953c7773c

This creates a merge commit that:

- Records all upstream commits as officially merged in git history
- Makes **no file changes** (all changes already applied via cherry-pick)
- Updates GitHub UI to show correct sync status (0 commits behind)

**Result:** Klaus Code is now officially 0 commits behind upstream in GitHub's UI, while maintaining all Klaus Code specific modifications
...
## Git Sync Status (After Strategy-Ours Merge)

After the fake merge commit (`e61d32b01`), git now correctly shows Klaus Code as synchronized:

```bash
$ git log main..roocode/main --no-merges | wc -l
0

# ✅ Git sees 0 "missing" commits
# ✅ GitHub UI shows: 76 commits ahead, 0 commits behind
```

**Before the merge:** Git showed 25 "missing" commits (cherry-picked with different hashes)
**After the merge:** Git correctly recognizes all upstream commits as merged
...
- Strategy-ours merge performed to update git history
- Git now shows 0 commits behind (GitHub UI accurate)
- Klaus Code has 76 additional commits (fork-specific work + merge)
...
**Created:** 2026-01-25
**Last Updated:** 2026-01-25 (after strategy-ours merge)
**Merge Commit:** `e61d32b01`
