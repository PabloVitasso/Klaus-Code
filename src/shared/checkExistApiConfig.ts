...
export function checkExistKey(config: ProviderSettings | undefined) {
	...
	// Special case for fake-ai, openai-codex, qwen-code, roo, and claude-code providers which don't need any configuration.
	if (
		config.apiProvider &&
		["fake-ai", "openai-codex", "qwen-code", "roo", "claude-code"].includes(config.apiProvider)
	) {
		return true
	}
